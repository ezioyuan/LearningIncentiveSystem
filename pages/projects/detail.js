...
class ProjectDetail extends React.Component {
  static async getInitialProps({ query }) {
    ...
    const [description, minInvest, maxInvest, goal, startTime, endTime, percent, balance,
      investorCount, paymentCount, owner] = Object.values(
      summary
    );
  ...
  async contributeProject() {
    ...
    // 字段合规检查
    if (amount <= 0) {
      return this.setState({ errmsg: '激励金额必须大于0' });
    }
    if (amount < minInvestInEther) {
      return this.setState({ errmsg: '激励金额必须大于最小激励金额' });
    }
    if (amount > maxInvestInEther) {
      return this.setState({ errmsg: '激励金额必须小于最大激励金额' });
    }
    ...
    try {
      ...
      this.setState({ errmsg: '激励成功', amount: 0 });
      console.log(result);
  ...
  async approvePayment(i) {
    try {
      ...
      if (!isInvestor) {
        return window.alert('只有参与激励才有权投票');
      }
  ...
  async doPayment(i) {
    try {
      ...
      if (sender !== this.props.project.owner) {
        return window.alert('只有目标制定者才能消费激励');
      }
    ...
    return (
      <Paper style={{ padding: '15px' }}>
        ...
        <Grid container spacing={16}>
          ...
          <InfoBlock title={`${project.startTime}`} description="目标执行时间" />
          <InfoBlock title={`${project.endTime}`} description="目标结束时间" />
        ...
        <Grid container spacing={16}>
          <Grid item md={12}>
            ...
            <Button size="small" variant="raised" color="primary" onClick={this.onSubmit}>
              {this.state.loading ? <CircularProgress color="secondary" size={24} /> : '立即激励'}
            </Button>
            {!!this.state.errmsg && (
    ...
    return (
      <Paper style={{ padding: '15px' }}>
        <Table style={{ marginBottom: '30px' }}>
          <TableHead>
            <TableRow>
              <TableCell>激励消费理由（目标进度）</TableCell>
              <TableCell numeric>请求激励金额</TableCell>
        ...
        <TableCell>
          ...
          {canDoPayment && (
            <Button size="small" color="primary" onClick={() => this.doPayment(index)}>
              {this.isPaying(index) ? <CircularProgress color="primary" size={24} /> : '激励划转'}
            </Button>
          )}
        </TableCell>
      </TableRow>
